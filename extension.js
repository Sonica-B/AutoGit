...
function shouldExcludeFile(filepath) {
    // Always exclude git internal files
    if (filepath.startsWith('.git/') || filepath.includes('/.git/')) {
        console.log(`Auto Git: Excluding git internal file: ${filepath}`);
        return true;
    }
    
    // Check extension exclude patterns
    ...
    const shouldExclude = excludePatterns.some(pattern => {
        try {
            ...
            if (matches) {
                console.log(`Auto Git: Excluding file ${filepath} (matches pattern: ${pattern})`);
            }
            return matches;
        } catch (regexError) {
...
function handleFileChange(uri, changeType) {
    ...
    // Skip git internal files immediately
    if (relativePath.startsWith('.git/') || relativePath.includes('/.git/')) {
        return;
    }
    
    // Check if file should be excluded by our patterns
